...
**The local code executor is not a security boundary.** Running model-authored or otherwise untrusted code safely requires one of the documented sandboxed backends (e.g. E2B, Docker, Modal). The local executor makes no isolation guarantee; escaping it, reading the host filesystem, or reaching the network from it is expected and out of scope. Reports against a *sandboxed* backend that demonstrate a real escape are in scope.
